...
class Hooks {
   ...
   */
  public static function addAction(string $tag, string $class, string $method = '__invoke', int $priority = 10, int $acceptedArgs = 1): void {
   ...
   * @param int $acceptedArgs The number of arguments the method accepts.
   */
  public static function addFilter(string $tag, string $class, string $method = '__invoke', int $priority = 10, int $acceptedArgs = 1): void {
    if (!method_exists($class, $method)) {
      throw new InvalidArgumentException("The method $method does not exist on $class");
    }
